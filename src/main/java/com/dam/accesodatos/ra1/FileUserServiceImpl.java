...
public class FileUserServiceImpl implements FileUserService {
    ...
    public String randomAccessRead(String filePath, long position, int length) {
        ...
        if (bytesLeidos == -1) {
            return "El archivo está vacio";// comprueba que el archivo tenga información
        } else {
            return new String(bf);
        } //Combierte el array de bytes a un string
    ...
    public boolean validateDirectoryStructure(String basePath) {
        ...
        for (File archivo : archivos) {
            if (archivo.isDirectory() && archivo.getName().equals("data")) {
                existeData = true;
            }
            if (archivo.isDirectory() && archivo.getName().equals("exports")) {
                existeExports = true;
            }
            if (archivo.isDirectory() && archivo.getName().equals("temp")) {
                existeTemp = true;
            }
        }
        ...
        try {
            ...
            if (!existeData) {
                ...
                if (!Files.isReadable(path)) {
                    directorioData.setReadable(true);
                }
                if (!Files.isWritable(path)) {
                    directorioData.setWritable(true);
                }
            ...
            if (!existeExports) {
                ...
                if (!Files.isReadable(path)) {
                    directorioExports.setReadable(true);
                }
                if (!Files.isWritable(path)) {
                    directorioExports.setWritable(true);
                }
            }
            if (!existeTemp) {
                ...
                if (!Files.isReadable(path)) {
                    directorioTemp.setReadable(true);
                }
                if (!Files.isWritable(path)) {
                    directorioTemp.setWritable(true);
                }
            }
    ...
    public String formatTextFile(String sourceFile) {
        ...
        try {
            File tempFile = File.createTempFile(sourceFile, ".txt");
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile));
                ...
                while (linea != null) {
                    ...
                    for (Character c : linea.toCharArray()) {
                        ...
                        if (primerCaracter && c == ' ') {
                            ...
                            lineaFinal.append(c.toString().toUpperCase());
                        } else {

                            if (ultimoCaracter != null && ultimoCaracter == ' ' && c == ' ') {
                            } else {
                                lineaFinal.append(c);
                            }
                            ultimoCaracter = c;
                    ...
                    writer.write(lineaFinal.toString().concat("\n"));
                    linea = reader.readLine();
                }
            ...
            e.printStackTrace();
            return "No se ha podido crear el archivo temporal";
        }
